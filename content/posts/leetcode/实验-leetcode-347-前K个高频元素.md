---
title: "实验-leetcode-347-前 K 个高频元素"
date: '2026-02-27'
draft: false
description:  
toc: true
...
实验编号/标题：例如：实验-leetcode-347-前 K 个高频元素

日期：Feb 27, 2026

所属领域/标签：例如：#LeetCode #堆
...
![](https://an-img.oss-cn-hangzhou.aliyuncs.com/2026/02/27/20260227202632794.png)
    ...
    public int[] topKFrequent(int[] nums, int k) {
        ...
        for (int num : freqMap.keySet()) {
            if (minHeap.size() < k) {
                ...
                if (freqMap.get(num) > freqMap.get(minHeap.peak())) {
                    minHeap.poll();
                    minHeap.offer(num);
                }
            }
    ...
    public int[] topKFrequent(int[] nums, int k) {
        ...
        for (int num : freqMap.keySet()) {
            if (minHeap.size() < k) {
                ...
                if (freqMap.get(num) > freqMap.get(minHeap.peek())) {
                    minHeap.poll();
                    minHeap.offer(num);
                }
            }
